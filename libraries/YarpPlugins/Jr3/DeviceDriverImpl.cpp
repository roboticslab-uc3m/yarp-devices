...
bool roboticslab::Jr3::open(yarp::os::Searchable& config)
{
    ...
    if ( ( fd = ::open("/dev/jr3",O_RDWR) ) < 0) {
        yError() << "Can't open device, no way to read force!";
        return false;
    }
    ...
    yInfo() << "Full scales of Sensor 0 are:" << fs0.f[0] << fs0.f[1] << fs0.f[2] << fs0.m[0] << fs0.m[1] << fs0.m[2];
    ret=ioctl(fd,IOCTL1_JR3_GET_FULL_SCALES,&fs1);
    yInfo() << "Full scales of Sensor 1 are:" << fs1.f[0] << fs1.f[1] << fs1.f[2] << fs1.m[0] << fs1.m[1] << fs1.m[2];
    ret=ioctl(fd,IOCTL2_JR3_GET_FULL_SCALES,&fs2);
    yInfo() << "Full scales of Sensor 2 are:" << fs2.f[0] << fs2.f[1] << fs2.f[2] << fs2.m[0] << fs2.m[1] << fs2.m[2];
    ret=ioctl(fd,IOCTL3_JR3_GET_FULL_SCALES,&fs3);
    yInfo() << "Full scales of Sensor 3 are:" << fs3.f[0] << fs3.f[1] << fs3.f[2] << fs3.m[0] << fs3.m[1] << fs3.m[2];
