...
bool TechnosoftIpos_ParamsParser::getParamValue(const std::string& paramName, std::string& paramValue) const
{
    if (paramName =="canId")
    {
    ...
    if (paramName =="useEmbeddedPid")
    {
        if (m_useEmbeddedPid==true) paramValue = "true";
    ...
    if (paramName =="max")
    {
        paramValue = std::to_string(m_max);
        return true;
    }
    if (paramName =="min")
    {
        paramValue = std::to_string(m_min);
        return true;
    }
    if (paramName =="maxVel")
    {
        paramValue = std::to_string(m_maxVel);
        return true;
    }
    if (paramName =="refSpeed")
    {
        paramValue = std::to_string(m_refSpeed);
        return true;
    }
    if (paramName =="refAcceleration")
    {
        paramValue = std::to_string(m_refAcceleration);
        return true;
    }
    if (paramName =="extraTr")
    {
        paramValue = std::to_string(m_extraTr);
        return true;
    }
    if (paramName =="samplingPeriod")
    {
        paramValue = std::to_string(m_samplingPeriod);
        return true;
    }
    ...
    if (paramName =="heartbeatPeriod")
    {
        paramValue = std::to_string(m_heartbeatPeriod);
        return true;
    }
    if (paramName =="syncPeriod")
    {
        paramValue = std::to_string(m_syncPeriod);
        return true;
    }
    if (paramName =="initialControlMode")
    {
        paramValue = m_initialControlMode;
        return true;
    }
    if (paramName =="sdoTimeout")
    {
        paramValue = std::to_string(m_sdoTimeout);
        return true;
    }
    if (paramName =="driveStateTimeout")
    {
        paramValue = std::to_string(m_driveStateTimeout);
        return true;
    }
    if (paramName =="tpdo1InhibitTime")
    {
        paramValue = std::to_string(m_tpdo1InhibitTime);
        return true;
    }
    if (paramName =="tpdo1EventTimer")
    {
        paramValue = std::to_string(m_tpdo1EventTimer);
        return true;
    }
    if (paramName =="tpdo2InhibitTime")
    {
        paramValue = std::to_string(m_tpdo2InhibitTime);
        return true;
    }
    if (paramName =="tpdo2EventTimer")
    {
        paramValue = std::to_string(m_tpdo2EventTimer);
        return true;
    }
    ...
    if (paramName =="driver::peakCurrent")
    {
        paramValue = std::to_string(m_driver_peakCurrent);
        return true;
    }
    if (paramName =="motor::k")
    {
        paramValue = std::to_string(m_motor_k);
        return true;
    }
    if (paramName =="gearbox::tr")
    {
        paramValue = std::to_string(m_gearbox_tr);
        return true;
    }
    if (paramName =="encoder::encoderPulses")
    {
        paramValue = std::to_string(m_encoder_encoderPulses);
        return true;
    }
    if (paramName =="ipMode")
    {
    ...
    if (paramName =="ipPeriodMs")
    {
        paramValue = std::to_string(m_ipPeriodMs);
        return true;
    }
    if (paramName =="enableIp")
    {
        if (m_enableIp==true) paramValue = "true";
        else paramValue = "false";
        return true;
    }
    if (paramName =="enableCsv")
    {
        if (m_enableCsv==true) paramValue = "true";
        else paramValue = "false";
        return true;
    }
    if (paramName =="initialInteractionMode")
    {
        paramValue = m_initialInteractionMode;
        return true;
    }
    if (paramName =="stiffness")
    {
        paramValue = std::to_string(m_stiffness);
        return true;
    }
    if (paramName =="damping")
    {
        paramValue = std::to_string(m_damping);
        return true;
    }
    if (paramName =="impedanceOffset")
    {
        paramValue = std::to_string(m_impedanceOffset);
        return true;
    }
    if (paramName =="minStiffness")
    {
        paramValue = std::to_string(m_minStiffness);
        return true;
    }
    if (paramName =="maxStiffness")
    {
        paramValue = std::to_string(m_maxStiffness);
        return true;
    }
    if (paramName =="minDamping")
    {
        paramValue = std::to_string(m_minDamping);
        return true;
    }
    if (paramName =="maxDamping")
    {
        paramValue = std::to_string(m_maxDamping);
        return true;
    }
    if (paramName =="kp")
    {
        paramValue = std::to_string(m_kp);
        return true;
    }
    if (paramName =="ki")
    {
        paramValue = std::to_string(m_ki);
        return true;
    }
    if (paramName =="kd")
    {
        paramValue = std::to_string(m_kd);
        return true;
    }
    if (paramName =="maxInt")
    {
        paramValue = std::to_string(m_maxInt);
        return true;
    }
    if (paramName =="maxOutput")
    {
        paramValue = std::to_string(m_maxOutput);
        return true;
    }
    if (paramName =="offset")
    {
        paramValue = std::to_string(m_offset);
        return true;
    }
    if (paramName =="scale")
    {
    ...
    if (paramName =="stictionUp")
    {
        paramValue = std::to_string(m_stictionUp);
        return true;
    }
    if (paramName =="stictionDown")
    {
        paramValue = std::to_string(m_stictionDown);
        return true;
    }
    if (paramName =="kff")
    {
        paramValue = std::to_string(m_kff);
        return true;
    }
    if (paramName =="errorLimit")
    {
        paramValue = std::to_string(m_errorLimit);
        return true;
    }
...
std::string TechnosoftIpos_ParamsParser::getConfiguration() const
{
