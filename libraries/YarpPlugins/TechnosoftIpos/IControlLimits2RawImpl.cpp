...
bool roboticslab::TechnosoftIpos::setMinLimitRaw(double min)
{
    ...
    int sendMin = min * abs(this->tr) * (encoderPulses / 360.0);  // Appply tr & convert units to encoder increments
...
bool roboticslab::TechnosoftIpos::setMaxLimitRaw(double max)
{
    ...
    int sendMax = max * abs(this->tr) * (encoderPulses / 360.0);  // Appply tr & convert units to encoder increments
