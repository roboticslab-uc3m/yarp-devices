...
{
    yCInfo(CBB) << "Initializing CanBusBroker" << type << "thread" << id;
}
...
{
    yCInfo(CBB) << "Configuring CanBusBroker" << type << "thread" << id << "->" << (success ? "success" : "failure");
}
...
{
    yCInfo(CBB) << "Stopping CanBusBroker" << type << "thread" << id;
}
