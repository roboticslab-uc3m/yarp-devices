...
bool teo::TechnosoftIpos::positionMoveRaw(int j, double ref)    // encExposed = ref;
{
    ...
    int position = ref * this->tr * (4096/360);  // Appply tr & convert units to encoder increments
...
bool teo::TechnosoftIpos::relativeMoveRaw(int j, double delta)
{
    ...
    int sendDelta = delta * this->tr * (4096/360);  // Appply tr & convert units to encoder increments
...
bool teo::TechnosoftIpos::setRefAccelerationRaw(int j, double acc)
{
    ...
    int sendRefAcc = acc * this->tr * (4096/360);  // Appply tr & convert units to encoder increments
