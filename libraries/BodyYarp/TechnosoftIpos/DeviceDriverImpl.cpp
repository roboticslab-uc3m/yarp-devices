...
bool teo::TechnosoftIpos::open(Searchable& config) {
    ...
    this->refAcceleration = config.check("refAcceleration",-1,"ref acceleration").asDouble();
    this->refSpeed = config.check("refSpeed",-1,"ref speed").asDouble();
    ...
    if( -1 == this->refAcceleration ) {
        CD_ERROR("Could not create TechnosoftIpos with refAcceleration -1\n");
        return false;
    }
    if( -1 == this->refSpeed ) {
        CD_ERROR("Could not create TechnosoftIpos with refSpeed -1\n");
        return false;
    }
