 ...
 *
 * @brief Firmware that allows communication with the PIC and the publication of
position messages.
 *
 ...
 *
 * Author: <a
href="http://roboticslab.uc3m.es/roboticslab/people/r-de-santos">Raul de Santos
Rico</a>
 *
 ...
 * Open the proyect with MPLAB, open "main.c" file and change the value of
"canId" variable (You can see <a class="el"
href="http://robots.uc3m.es/index.php/CuiAbsolute_Documentation">correspondence</a>)
<br>
 * Also, to increase the time between sending messages,  you can modify
"sendDelay" value too (see code documentation in main.c).
 * Then, build all. <br>
 * Finally, select programmer "MPLAB ICD 2", disconnect CAN-BUS connections of
the PIC, connect and program it. <br>
 *
 * @section cuiAbsolute_running Running (assuming correct installation)
 *
 *
 * To test Cui Aboslute encoders are working properly, you need to start the
program named "testCuiAbsolute" and see that the test passed correctly. <br>
 * Previously, you have to manually change the ID of Cui that you want to test
in the code (example: #define CAN_ID 124), compile and reinstall. <br>
...
BYTE x, y;
ECAN_TX_MSG_FLAGS txFlags =
    ECAN_TX_PRIORITY_3 & ECAN_TX_STD_FRAME & ECAN_TX_NO_RTR_FRAME;
...
void main(void) {
  ...
  */
  ...
  /* Manual (page 44):
  This macro sets the value for a mask register. There are a total of two
  macros, one for each mask.
  For example, for mask RXM0, use ECANSetRXM0Value, for RXM1, use
  ECANSetRXM1Value and so on
  ...
  ECANSetRxBnRxMode(RXB0, ECAN_RECEIVE_ALL_VALID); // ECAN_RECEIVE_ALL_VALID
                                                   // (funciona)
                                                   // ECAN_RECEIVE_STANDARD
                                                   // (inestable, se queda
                                                   // pillado) ECAN_RECEIVE_ALL
                                                   // (no funciona, el mensaje
                                                   // de STOP lo ignora)

  // Manual (page 36): This macro sets the CAN bus Wake-up Filter mode.
  // Specifies that the low-pass filter be disabled
...
void send() {
  // - Manual: [1. The host issues the command, 0x10. The data read in at this
  // time will be 0xa5 or 0x00 since this is the first SPI transfer]
  LATCbits.LATC2 = 0;
  WriteSPI(0b00010000); // Solicitación de posición (comando 0x10)
  Delay10TCYx(5); // Used 4 to fix Known error message (37 b6 ff c4) or (3c 13
                  // fe c4). Old delay: 3 ( 3 = 6us)
  y = SSPBUF; // Recoge los datos del SSPBUF que provienen del encoder (MISO)
  LATCbits.LATC2 = 1;

  // - Manual: [2. The host waits a minimum of 5 µs then sends a “nop_a5”
  // command: 0x00]
  LATCbits.LATC2 = 0;
  ...
  // - Manual: [3. The response to the “nop_a5” is either 0xa5 or an echo of the
  // command, 0x10.]
  while (y ==
         0b10100101) // - Manual: [a. If it is 0xa5, it will go back to step 2.]
  {
    Delay10TCYx(5);
    LATCbits.LATC2 = 0;
    WriteSPI(0b00000000); // Comando 0x00 -> nop_a5)
  ...
  // - Manual: [4. The host waits a minimum of 5 µs then sends “nop_a5”, the
  // data read is the high byte of the position.]
  LATCbits.LATC2 = 0;
  WriteSPI(0b00000000); // Comando 0x00 -> nop_a5
  Delay10TCYx(5); // Used 4 to fix Known error message (37 b6 ff c4) or (3c 13
                  // fe c4). Old delay: 3 ( 3 = 6us)
  y = SSPBUF; // Recoge los datos del SSPBUF que provienen del encoder (MISO)
  message[0] =
      y; // Se almacena en el primer elemento del array que se enviará por CAN
  LATCbits.LATC2 = 1;

  // - Manual: [5. The host waits a minimum of 5 µs then sends “nop_a5”, the
  // data read is the low byte of the position.]
  LATCbits.LATC2 = 0;
  ...
  y = SSPBUF; // Recoge los datos del SSPBUF que provienen del encoder (MISO)
  message[1] =
      y; // Se almacena en el segundo elemento del array que se enviará por CAN

  // - Manual: [6. The host waits a minimum of 5 µs before sending another SPI
  // command.]
  LATCbits.LATC2 = 1;
  aux = (message[0] << 8) + message[1]; // Se rota el byte alto de la posición 8
                                        // bits (2^8=256) y se suma al bajo
