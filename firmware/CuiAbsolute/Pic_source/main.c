 ...
 * Open the proyect with MPLAB, open "main.c" file and change the value of "canId" variable spondence</a>) <br>
...
// -- Inicializacion de variables para el envio
BYTE delay;
int aux, message[2];
double degrees;
BYTE x, y;
...
int pushFlag = 0; // flag para saber si esta ejecutando el PUSH

// -- Variables que almacenaran los datos a recibir:
unsigned long picId;
BYTE data[8];
BYTE dataLen;
...
void main(void) {
  ...
  */
  ...
  -- data[0]: (Byte 1)
      * start push (0x01) publicacion permanente (necesita indicar periodo de publicacion)
      * start pull (0x02) publicacion por peticion (no necesita indicar velocidad de publicacion)
      * stop  push (0x03) para la publicación por push
  -- data[1]: (Byte 2)
      * Periodo de publicacion entre mensajes usando publicacion tipo push.
  Valores (0 - 255)
      A tener en cuenta:
...
void sendData(BYTE op) {
  ...
  LATCbits.LATC2 = 0;
  WriteSPI(0b00010000); // Solicitacion de posicion (comando 0x10)
  Delay10TCYx(3); // Used 4 to fix Known error message (37 b6 ff c4) or (3c 13
                  // fe c4). Delay10TCYx(3)= 6us
  ...
  y = SSPBUF; // Recoge los datos del SSPBUF que provienen del encoder (MISO)
  message[0] =
      y; // Se almacena en el primer elemento del array que se enviara por CAN
  LATCbits.LATC2 = 1;
  ...
  y = SSPBUF; // Recoge los datos del SSPBUF que provienen del encoder (MISO)
  message[1] =
      y; // Se almacena en el segundo elemento del array que se enviara por CAN
